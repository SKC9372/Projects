...
FROM
    movie
WHERE
    movie_id IS NULL OR title IS NULL
        OR year IS NULL
...
FROM
    ...
    ratings
WHERE
    country = 'Italy' OR country = 'Germany'
GROUP BY country
ORDER BY votes DESC;
...
-- Type your code below:
SELECT 
    name AS director_names, COUNT(title) AS movie_count
FROM
    movie
        INNER JOIN
    ratings USING (movie_id)
        INNER JOIN
    director_mapping USING (movie_id)
        INNER JOIN
    names USING (name_id)
WHERE
    avg_rating > 8
...
-- Type your code below:
SELECT 
    name AS actor_names, COUNT(title) AS movie_count
FROM
    movie
        INNER JOIN
    ratings USING (movie_id)
...
WITH top_actor AS (
    SELECT
        name AS actor_name,
        SUM(total_votes) AS actor_votes,
        COUNT(title) AS movie_count,
    ...
    FROM
        ...
        INNER JOIN names USING (name_id)
    WHERE
        country = 'INDIA'
    GROUP BY
        actor_name
...
SELECT
    *,
    RANK() OVER (ORDER BY actor_avg_rating DESC, actor_votes DESC) AS actor_rank
FROM
    top_actor;
...
WITH top_actresses AS (
    SELECT
        ...
        SUM(total_votes) AS actress_votes,
        COUNT(title) AS movie_count,
...
SELECT
    *,
    RANK() OVER (ORDER BY actress_avg_rating DESC, actress_votes DESC) AS actor_rank
FROM
    top_actresses;
...
Romance	109.5342	1141.5291	103.77537273
Crime	107.0517	424.4287	106.10717500
Drama	106.7746	531.2033	106.24066000
Fantasy	105.1404	737.3106	105.33008571--+*/
-- Type your code below:
...
WITH movie_summary AS (
    SELECT
        ...
        RANK() OVER (PARTITION BY year, genre ORDER BY worlwide_gross_income DESC) AS movie_rank
    FROM
        movie
        INNER JOIN genre USING (movie_id)
)
...
WITH top_actress AS (
    ...
    FROM
        ...
        INNER JOIN names USING (name_id)
    WHERE
        genre = 'Drama' AND category = 'Actress'
    GROUP BY
        name, total_votes
    HAVING
        actress_avg_rating > 8
)
SELECT
    *
FROM
...
WITH date_summary AS (
    SELECT
        ...
        name,
        date_published,
        LAG(date_published, 1) OVER(PARTITION BY name_id ORDER BY date_published, movie_id) AS date_lead
    FROM
        movie
        INNER JOIN director_mapping USING (movie_id)
        INNER JOIN names USING (name_id)
        INNER JOIN ratings USING (movie_id)
),
date_difference AS (
    SELECT
        *,
        DATEDIFF(date_published, date_lead) AS date_diff
    FROM
        date_summary
),
average_int_days AS (
    SELECT
        name_id,
        AVG(date_diff) AS avg_inter_days
    FROM
        date_difference
    GROUP BY
        name_id
...
SELECT
    ...
    ROUND(avg_inter_days, 2) AS avg_inter_days,
    ROUND(AVG(avg_rating), 2) AS avg_rating,
    MIN(avg_rating) AS min_avg_rating,
    MAX(avg_rating) AS max_avg_rating,
    SUM(duration) AS total_duration,
    ROW_NUMBER() OVER (ORDER BY COUNT(title) DESC) AS director_rank
FROM
    movie
    INNER JOIN ratings USING (movie_id)
    INNER JOIN director_mapping USING (movie_id)
    INNER JOIN names USING (name_id)
    INNER JOIN average_int_days USING (name_id)
GROUP BY
    name_id
LIMIT 9;
